    ...
    try {
      ...
      if (friendRequests && friendRequests.length > 0) {
        ...
        friendRequestsWithSenders = friendRequests.map(request => ({
          ...request,
          sender: senders?.find(sender => sender.id === request.sender_id) || { username: 'Unknown User' }
        }));
      }
      ...
      const combinedNotifications: NotificationItem[] = [
        ...
        ...friendRequestsWithSenders.map(request => ({
          ...
          friendRequest: {
            ...request,
            sender: request.sender || { id: request.sender_id, username: 'Unknown User', avatar_url: undefined }
          },
        })),
      ];
    ...
    switch (notification.type) {
      ...
      case 'friend_accepted':
        if (notification.data?.friend_id) {
          navigation.navigate('UserProfile', { userId: notification.data.friend_id });
        }
        break;
      case 'message':
        if (notification.data?.sender_id) {
          // Navigate to messaging screen - we'd need to get the sender's username first
          navigation.navigate('UserProfile', { userId: notification.data.sender_id });
        }
        break;
    }
  };
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              setNotifications(prev => 
                prev.filter(notification => notification.type === 'friend_request')
              );
          ...
          {item.type === 'friend_request' && item.friendRequest && (
            <View style={styles.friendRequestActions}>
              <View style={styles.senderInfo}>
                <Image
                  source={{
                    uri: item.friendRequest.sender.avatar_url || 'https://via.placeholder.com/40',
                  }}
                  style={styles.senderAvatar}
                />
                <Text style={styles.senderName}>{item.friendRequest.sender.username}</Text>
              </View>
              
              <View style={styles.actionButtons}>
                <TouchableOpacity
                  style={[styles.actionButton, styles.acceptButton]}
                  onPress={() => handleFriendRequest(item.friendRequest!.id, 'accept')}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleFriendRequest(item.friendRequest!.id, 'decline')}
                >
